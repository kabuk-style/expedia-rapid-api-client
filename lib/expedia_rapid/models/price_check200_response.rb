  ...
  class PriceCheck200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ExpediaRapid::PriceCheck200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ExpediaRapid::PriceCheck200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'amount_owed')
        self.amount_owed = attributes[:'amount_owed']
      end
    ...
    def hash
      [status, occupancy_pricing, links, card_on_file_limit, refundable_damage_deposit, deposits, refund, amount_owed, penalty, trader_information].hash
    end
