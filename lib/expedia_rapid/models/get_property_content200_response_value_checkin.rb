...
module ExpediaRapid
  ...
  class GetPropertyContent200ResponseValueCheckin
    # Present if the property has 24-hour check-in.
    attr_accessor :_24_hour
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ExpediaRapid::GetPropertyContent200ResponseValueCheckin` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ExpediaRapid::GetPropertyContent200ResponseValueCheckin`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'_24_hour')
        self._24_hour = attributes[:'_24_hour']
      end
    ...
    def hash
      [_24_hour, begin_time, end_time, instructions, special_instructions, min_age].hash
    end
